...
struct LayoutManagerView: View {
    ...
    private func createNewLayout() {
        ...
        openEditor(for: newLayout, screen: screen)
    }
    ...
    private func editLayout(_ layout: CustomLayout) {
        ...
        openEditor(for: layout, screen: screen)
    }

    private func openEditor(for layout: CustomLayout, screen: NSScreen) {
        let editorView = LayoutEditorView(layout: layout, screen: screen)
        let hostingController = NSHostingController(rootView: editorView)

        let window = NSWindow(contentViewController: hostingController)
        window.title = "Layout Editor - \(layout.name)"
    ...
    private var presetTemplates: [PresetTemplate] {
        [
            ...
            PresetTemplate(name: "Three Rows", iconName: "rectangle.split.1x2"),
            ...
            PresetTemplate(name: "Coding Setup Reverse", iconName: "arrow.left.arrow.right")
        ]
    }
}
